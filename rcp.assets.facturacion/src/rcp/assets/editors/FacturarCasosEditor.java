...
public class FacturarCasosEditor extends AbstractEditor {

	public static final String ID = "rcp.assets.editors.FacturarCasosEditor"; //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
				...
				btnBuscar.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						...
						if (!txtNoCaso.getText().equals("")) {
							Long noCaso = Long.valueOf(txtNoCaso.getText().trim());								
			...
			}
			{
				...
				mghprlnkFactura.addHyperlinkListener(new IHyperlinkListener() {
					...
					public void linkActivated(HyperlinkEvent e) {
						Caso caso = getElementoSeleccionado(viewer);
						if (caso != null) {
							if (!caso.isFacturable()) {
								MessageDialog.openError(getSite().getShell(), "Generar factura", "El caso número " + caso.getNoCaso() + " tiene un estado que no permite generar una factura (Estado: " + caso.getDspEstado() + ").");
							} else 	if (facturasController.existeFactura(caso)) {
								...
								boolean flagAbrirFactura = MessageDialog.openQuestion(getSite().getShell(), "Generar factura", "Ya existe una factura para el caso seleccionado.  Para generar una nueva\n" +
										"factura debe anular el documento existente (número " + facturaExistente.getNoFactura() + ").  Desea abrir la\n" +
										"factura existente?");
								...
								boolean flagGenerarFactura = MessageDialog.openConfirm(getSite().getShell(), "Generar factura", "Desea generar la factura del caso " + caso.getNoCaso() + "?");
								if (flagGenerarFactura) {
									Factura factura = generarNuevaFactura(caso);
									if (!caso.getEstado().equals("FA")) {
										caso.setEstado("FA");
										caso.setDspEstado("Facturado");
		...
		}
		{
			...
			}
			{
				...
				tblclmnNombreExpediente.setWidth(210);
	...
	private void generarProformaCaso(Caso caso) {
		try {							
			ProformasEditorInput input = new ProformasEditorInput(TipoProforma.CASO.getTitulo());
			input.setTipoProforma(TipoProforma.CASO.getNombre());
			// preparamos los parametros para una proforma de caso...
			Integer idCaso = Integer.valueOf(caso.getIdCaso().toString());
			Float impuesto = casosController.calcularImpuestos(caso, (float) 7F);
			String idioma = "1";
	...
	private Factura generarNuevaFactura(Caso caso) {
		System.out.println("ATENCION> " + caso.getAtencion());
		...
		FacturaCaso factura = casosController.generarFactura(caso, fechaFactura, numeroFactura, idiomaFactura, (float) 7F);
		//factura.setImpuesto(casosController.calcularImpuestos(caso,));
